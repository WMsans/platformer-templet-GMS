		...
		if(instance_exists(map[?ANIM_DATA.TARGET]) || map[?ANIM_DATA.TARGET]==global){
			if(map[?ANIM_DATA.DELAY]<=0){
				if(map[?ANIM_DATA.TIME] < map[?ANIM_DATA.DURATION]){
					...
					var value=map[?ANIM_DATA.START]+map[?ANIM_DATA.CHANGE]*scr_anim_getvalue(map[?ANIM_DATA.TWEEN],map[?ANIM_DATA.EASE],map[?ANIM_DATA.TIME]/map[?ANIM_DATA.DURATION],map[?ANIM_DATA.ARG_0],map[?ANIM_DATA.ARG_1]);
